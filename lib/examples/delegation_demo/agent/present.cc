...
bool RunAgentPresentCommand(const std::filesystem::path& delegation_dir,
  ...
  // 2. 读取验证请求。纯国密业务路径只接受 SM2/SM3 delegated profile。
  ...
  if (request.zk_system != "zk-agentauth-sm-delegation-v1") {
    if (err != nullptr) {
      *err = "unsupported request profile: " + request.zk_system;
    }
    return false;
  }
  ...
  Policy policy;
  if (!ReadDelegationSmDir(delegation_dir, &holder, &agent_sk,
                           &device_pkx_sm2, &device_pky_sm2, &agent_pkx_sm2,
                           &agent_pky_sm2, &del_msg_sm3, &del_sig_sm2,
                           &revocation_status_sm2, &policy, err)) {
    return false;
  }
  TrimAsciiWhitespace(&agent_sk);
